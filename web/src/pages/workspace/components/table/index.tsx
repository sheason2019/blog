...
const Table: Component<Props> = (props) => {
  const contentRender = createMemo(() => {
  ...
  return (
    <div class="w-full border border-gray-400 rounded-md overflow-hidden">
      <table class="w-full">
        <thead class="w-full">
          <tr class="w-full flex bg-gray-100 py-3">
            <For each={props.columns}>
              {(item) => <th class="flex-1 text-left px-2">{item.title}</th>}
            </For>
          </tr>
        </thead>
